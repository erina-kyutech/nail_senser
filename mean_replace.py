...
# ===== 平均画像置換設定 =====
USE_MEAN_REPLACE = True

REPLACE_LEFT_PIXELS = 40
REPLACE_RIGHT_PIXELS = 0
REPLACE_TOP_PIXELS = 0
REPLACE_BOTTOM_PIXELS = 0

# 平均画像を作るとき、train 全部使うなら None
# 重いなら 20000 などにする
MEAN_IMAGE_MAX_SAMPLES = None

# ===== 保存先を黒塗り版と分ける =====
PROJECT_ROOT = Path(r"C:\Users\Owner\PycharmProjects")
DATA_ROOT = PROJECT_ROOT / "datas"
RESULT_DIR = PROJECT_ROOT / "result" / "CNN_result" / "ifuku_train_test_rgb_mean_replace_left40"
...
# 平均画像置換
# =========================================================
def apply_mean_replace(img: np.ndarray, mean_img: np.ndarray | None) -> np.ndarray:
    """
    img, mean_img: (H,W,3) uint8
    指定領域を平均画像で置換
    """
    if mean_img is None:
        return img.copy()
    ...
    if REPLACE_LEFT_PIXELS > 0:
        out[:, :REPLACE_LEFT_PIXELS, :] = mean_img[:, :REPLACE_LEFT_PIXELS, :]

    if REPLACE_RIGHT_PIXELS > 0:
        out[:, -REPLACE_RIGHT_PIXELS:, :] = mean_img[:, -REPLACE_RIGHT_PIXELS:, :]

    if REPLACE_TOP_PIXELS > 0:
        out[:REPLACE_TOP_PIXELS, :, :] = mean_img[:REPLACE_TOP_PIXELS, :, :]

    if REPLACE_BOTTOM_PIXELS > 0:
        out[-REPLACE_BOTTOM_PIXELS:, :, :] = mean_img[-REPLACE_BOTTOM_PIXELS:, :, :]

    return out


def build_mean_image_from_df(df: pd.DataFrame, loader, max_samples: int | None = None) -> np.ndarray:
    """
    train画像から平均画像を作る
    """
    if len(df) == 0:
    ...
    if max_samples is not None and len(df) > max_samples:
        use_df = df.sample(n=max_samples, random_state=RANDOM_SEED).reset_index(drop=True)
    else:
        use_df = df.reset_index(drop=True)

    acc = np.zeros((IMG_H, IMG_W, IMG_C), dtype=np.float64)

    for i, path in enumerate(use_df["img_path"].tolist()):
        img_path = resolve_img_path(path)

        bgr = cv2.imread(str(img_path))
        if bgr is None:
            raise FileNotFoundError(f"image not found while building mean image: {img_path}")

        x = build_input_image(bgr, mode=loader.img_mode)
        ...
        if (i + 1) % 1000 == 0:
            print(f"[mean image] processed {i+1}/{len(use_df)}")

    mean_img = np.clip(acc / len(use_df), 0, 255).astype(np.uint8)
    return mean_img
...
class DataLoader:
    def __init__(self, name: str = "ifuku", img_mode: str = "rgb", mean_img: np.ndarray | None = None):
        self.name = name
    ...
    def load_one_image(self, path: str) -> np.ndarray:
        ...
        if USE_MEAN_REPLACE:
            x = apply_mean_replace(x, self.mean_img)
...
def preview_before_after(df: pd.DataFrame, loader: DataLoader, save_dir: Path, n_samples: int = 6) -> None:
    ...
    for i in range(n_samples):
        ...
        if USE_MEAN_REPLACE:
            after = apply_mean_replace(before, loader.mean_img)
        else:
            after = before.copy()
...
def main():
    ...
    # まずはmean_imgなしで一覧だけ読む
    loader = DataLoader(name=SUBJECT_NAME, img_mode=IMG_MODE, mean_img=None)
    ...
    # ===== 平均画像を train だけで作る =====
    print("=== build mean image from train data ===")
    tmp_loader = DataLoader(name=SUBJECT_NAME, img_mode=IMG_MODE, mean_img=None)
    mean_img = build_mean_image_from_df(train_df, tmp_loader, max_samples=MEAN_IMAGE_MAX_SAMPLES)

    mean_img_save_path = RESULT_DIR / "preview" / "train_mean_image.png"
    if IMG_MODE == "rgb":
        cv2.imwrite(str(mean_img_save_path), cv2.cvtColor(mean_img, cv2.COLOR_RGB2BGR))
    else:
        cv2.imwrite(str(mean_img_save_path), mean_img)
    print(f"[INFO] mean image saved -> {mean_img_save_path}")

    # mean_imgを持ったloaderを作り直す
    loader = DataLoader(name=SUBJECT_NAME, img_mode=IMG_MODE, mean_img=mean_img)

    # preview
    print("=== preview mean replaced images ===")
