...
class multitask_CNN(object):
    ...
    def CNN_build(self):
        ...
        # ===== ここだけ変更：GlobalMaxPooling2D -> Reshape + LSTM（追加）=====
        # convの出力は (None, H, W, C) になる（例: (None, 4, 4, 512)）
        # Hを時系列長として扱い、(None, H, W*C) にしてLSTMへ入れる
        conv_shape = K.int_shape(conv)  # (None, H, W, C)
        H = conv_shape[1]
        W = conv_shape[2]
        C = conv_shape[3]
        reshape_features = W * C

        # (None, H, W, C) -> (None, H, W*C)
        reshape = Reshape((H, reshape_features), name="reshape_for_lstm")(conv)

        # LSTMで時系列特徴を集約 -> (None, 128) ※unitsは固定値（必要なら変更可）
        lstm_out = LSTM(128, name="lstm_layer")(reshape)

        # LSTM出力をflattenの代わりとして使う（構造は“全結合の入口が変わっただけ”）
        flatten = lstm_out
        # ===== 変更ここまで =====
