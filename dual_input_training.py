...
class Trainer(object):
    ...
    def base_train(self):
        ...
        index_array = np.random.permutation(
            np.arange(data_length, dtype="uint32")
        )

        train_rate = 0.85
        split_point = int(len(index_array) * train_rate)
        train_idx = index_array[:split_point]
        test_idx  = index_array[split_point:]

        train_df = personaldatas_df.iloc[train_idx].reset_index(drop=True)
        test_df  = personaldatas_df.iloc[test_idx].reset_index(drop=True)
        ...
        self.evaluate_save(test_df, keyword="val")
