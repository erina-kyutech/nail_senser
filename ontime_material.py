...
CAM_LEFT_INDEX = 1   # nail
CAM_RIGHT_INDEX = 0  # tip
...
# 常に最新のCSV(datalog_fr_dualpath.csv)は毎フレーム即時記録されているので、
# 万一Excel保存が間に合わなくてもデータが失われることはない。
EXCEL_EXPORT = False
EXCEL_PATH = "./datalog_fr_dualpath.xlsx"
...
class RealTime:
    def __init__(self):
        ...
        # csv
        self.datalog_path = "./datalog_fr_dualpath.csv"
        ...
        print("\n準備OK：指の位置を確認して 'r' を押すと測定開始（'ESC'で終了）")
    ...
    def _export_excel_async(self, rows_snapshot):
        ...
        t = threading.Thread(target=_worker, args=(rows_snapshot, self.header, EXCEL_PATH), daemon=True)
        t.start()
        return t
    ...
    def loop(self):
        try:
            while True:
                ...
                # ---- display（マスク適用後の画像 + FPS。素材確率はグラフ側のパネルに表示） ----
                disp_bgr = masked_bgr.copy()
                disp_bgr = cv2.resize(disp_bgr, (OUT_W * 2, OUT_H * 2))
                cv2.putText(disp_bgr, f"FPS: {self.fps_ema:.1f}  YOLO: {yolo_ms:.0f}ms",
