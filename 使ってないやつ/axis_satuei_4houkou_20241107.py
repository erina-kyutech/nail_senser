...
class SC800IM700_1:
  ...
  #共有メモリ変数を更新し続ける
  @classmethod
  def sub_loop(cls, ser_port,I2C_address,ser_flag, shear_force1):
...
class SC800IM700_2:
  ...
  #共有メモリ変数を更新し続ける
  @classmethod
  def sub_loop(cls, ser_port,I2C_address,ser_flag, shear_force2):
    ser = serial.Serial(ser_port,
                        baudrate=9600,
                        bytesize=8,
                        parity='N',
...
class SC800IM700_3:
  ...
  def single_read(self):
    ...
    if self.gain == 'x16':
      gf_data = (data_row * 10 - 24024) / 11312 # 3番
      gf_data = gf_data * (gf_data > 0)
  ...
  #共有メモリ変数を更新し続ける
  @classmethod
  def sub_loop(cls, ser_port,I2C_address,ser_flag, shear_force3):
    ser = serial.Serial(ser_port,
                        baudrate=9600,
                        bytesize=8,
                        parity='N',
...
class SC800IM700_4:
  ...
  def single_read(self):
    ...
    if self.gain == 'x16':
      gf_data = (data_row*10-15645)/11428  # 4番
      gf_data = gf_data * (gf_data > 0)
  ...
  #共有メモリ変数を更新し続ける
  @classmethod
  def sub_loop(cls, ser_port,I2C_address,ser_flag, shear_force4):
    ser = serial.Serial(ser_port,
                        baudrate=9600,
                        bytesize=8,
                        parity='N',
...
if __name__ == "__main__":
  ...
  sub_xy1 = Process(target=SC800IM700_1.sub_loop,
                   args=[xy_port_1,xy_address_1,ser_flag,shear_force1])
  sub_xy2 = Process(target=SC800IM700_2.sub_loop,
                   args=[xy_port_2,xy_address_2,ser_flag,shear_force2])
  sub_xy3 = Process(target=SC800IM700_3.sub_loop,
                   args=[xy_port_3,xy_address_3,ser_flag,shear_force3])
  sub_xy4 = Process(target=SC800IM700_4.sub_loop,
                   args=[xy_port_4,xy_address_4,ser_flag,shear_force4])
