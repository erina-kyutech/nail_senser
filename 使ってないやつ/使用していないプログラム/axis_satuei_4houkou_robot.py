...
class SC800IM700_4:
  ...
  def single_read(self):
    ...
    if self.gain == 'x16':
      gf_data = (data_row*10+19853)/16789  # 4番
      gf_data = gf_data * (gf_data > 0)
...
class graphmake:
  def __init__(self):
    ...
    self.axL.tick_params(axis='both', labelsize=15)

    # 中央グラフ(2番目のグラフ)
    self.axM = self.fig.add_subplot(gs[:,2:3], xlabel="xForce", ylabel="zForce")

    # 右側グラフ(3番目のグラフ)
    self.axR = self.fig.add_subplot(gs[:,3:5], xlabel="sayuu", ylabel="zengo")
    # --------------------------axM設定-------------------------------------------
    self.axM.set_xlim(-1, 1)  # x軸表示範囲
    self.axM.set_ylim(-0.5, 10.5)  # y軸表示範囲
    self.axM.xaxis.set_ticks_position("bottom")  # 目盛りを軸の下に表示
    self.axM.tick_params(axis='both', labelsize=15)
    ...
    self.axR.tick_params(axis='both', labelsize=15)
