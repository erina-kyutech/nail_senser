# -*- coding: utf-8 -*-
"""
4パターンの入力画像でVGG16ベースの3軸力推定モデルを学習・評価するスクリプト。

パターン:
  1. original          : 元画像(マスクなし)
  2. nail_only         : 爪のみマスク済み
  3. nail_and_tip      : 爪+指先マスク済み
  4. finger_tip_only   : 指先のみマスク済み
"""
...
DATA_ROOTS = {
    ...
    "finger_tip_only" : DATA_ROOT / "masked_finger_tip_only",
}
...
class DataLoader:
    ...
    def personal_dataload(self) -> pd.DataFrame:
        ...
        for _, row in names.iterrows():
            ...
            csv_record["img_path"] = csv_record["img_path"].apply(self._replace_root)
...
class ImageSequence(tf.keras.utils.Sequence):
    def __init__(self, df, loader, batch_size=32, shuffle=True):
...
def build_model() -> Model:
    ...
    model = Model(input_tensor, [head(flatten, "Fz"),
                                  head(flatten, "Fx"),
                                  head(flatten, "Fy")])
    model.compile(
...
def run_one_pattern(pattern_name: str, data_root: Path):
    ...
    print(f"train: {len(train_df)} 枚 | val: {len(val_df)} 枚 | test: {len(test_df)} 枚")
    ...
    if weight_path.exists():
        ...
        print("学習開始...")
        history = model.fit(
            train_seq, epochs=EPOCHS,
            validation_data=val_seq, verbose=1
        )
